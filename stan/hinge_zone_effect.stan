...
parameters {
  ...
  real d_beta_1_male;
  real d_beta_2_male;
  // zone effects for each fixed effect
  real d_alpha_zone;
  real d_eta_zone;
  real d_beta_1_zone;
  real d_beta_2_zone;
...
transformed parameters {
    ...
    for (i in 1:n_obs) {
      // intercepts
      IN[i] = alpha + d_alpha_zone*zone[i] + d_alpha_male*is_m[i] + u_fish[id_fish[i]] + u_year[id_year[i]] ;
      // thresholds 
      TR[i] = eta + d_eta_zone*zone[i] + d_eta_male*is_m[i];
      // pre-threshold slopes
      B1[i] = beta_1 + d_beta_1_zone*zone[i] + d_beta_1_male*is_m[i];
      // post-threshold slopes
      B2[i] = beta_2 + d_beta_2_zone*zone[i] + d_beta_2_male*is_m[i];
