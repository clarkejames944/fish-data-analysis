...
transformed parameters {
    ...
    for (i in 1:n_obs) {
      // intercepts
      IN[i] = alpha + d_alpha_EBS*is_EBS[i] + d_alpha_ETAS*is_ETAS[i] + d_alpha_WTAS*is_WTAS[i] + d_alpha_temp*(temp[i]-mean_temp) + d_alpha_male*is_m[i] + u_fish[id_fish[i]] + u_year[id_year[i]];
      // thresholds 
      TR[i] = eta + d_eta_EBS*is_EBS[i] + d_eta_ETAS*is_ETAS[i] + d_eta_WTAS*is_WTAS[i] + d_eta_temp*(temp[i]-mean_temp) + d_eta_male*is_m[i];
      // pre-threshold slopes
      B1[i] = beta_1 + d_beta_1_EBS*is_EBS[i] + d_beta_1_ETAS*is_ETAS[i] + d_beta_1_WTAS*is_WTAS[i] + d_beta_1_temp*(temp[i]-mean_temp) + d_beta_1_male*is_m[i];
      // post-threshold slopes
      B2[i] = beta_2 + d_beta_2_EBS*is_EBS[i] + d_beta_2_ETAS*is_ETAS[i] + d_beta_2_WTAS*is_WTAS[i] + d_beta_2_temp*(temp[i]-mean_temp) + d_beta_2_male*is_m[i];
