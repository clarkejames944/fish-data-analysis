...
transformed parameters {
    ...
    for (i in 1:n_obs) {
      // intercepts
      IN[i] = alpha + d_alpha_zone*zone[i] + d_alpha_temp*(temp[i]-mean_temp) + d_alpha_male*is_m[i] + u_fish[id_fish[i]] + u_year[id_year[i]];
      // thresholds 
      TR[i] = eta + d_eta_zone*zone[i] + d_eta_temp*(temp[i]-mean_temp) + d_eta_male*is_m[i];
      // pre-threshold slopes
      B1[i] = beta_1 + d_beta_1_zone*zone[i] + d_beta_1_temp*(temp[i]-mean_temp) + d_beta_1_male*is_m[i];
      // post-threshold slopes
      B2[i] = beta_2 + d_beta_2_zone*zone[i] + d_beta_2_temp*(temp[i]-mean_temp) + d_beta_2_male*is_m[i];
...
model { 
  ...
  d_alpha_zone ~ normal(0.0, 0.25);
  // threshold terms
  eta ~ normal(1.0, 1.00);
  d_eta_male ~ normal(0.0, 0.25);
  d_eta_temp ~ normal(0.0, 0.25);
  d_eta_zone ~ normal(0.0, 0.25);
  // slope terms
  ...
  d_beta_1_zone ~ normal(0.0, 0.25);
  d_beta_2_zone ~ normal(0.0, 0.25);
