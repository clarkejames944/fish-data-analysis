...
parameters {
    ...
    vector[3] beta;///obviously then we have just two slope values for all the groups
}
...
transformed parameters {
    ...
    for (i in 1:N_EBSm){
         yhat[i] = intercept[fishID[i]] + beta[1] * prev[i] + (beta[2] * (prev[i] - bp[fishID[i]]) * x2[i]) + (beta[3]*x2[i]);
    }
}
