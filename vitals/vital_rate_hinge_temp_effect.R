...
g_z1z_1 <- function(z1, z0, u, a, is_m, m_par) {
  with(m_par, {
    # intercept
    IN = alpha + d_alpha_male * is_m + d_alpha_temp * temp + u + u_year
    # threshold
    TR = eta + d_eta_male * is_m + d_eta_temp * temp
    # pre-threshold slope
    B1 = beta_1 + d_beta_1_male * is_m + d_beta_1_temp * temp
...
g_z1z_2 <- function(z1, z0, u, a, is_m, m_par) {
  with(m_par, {
    ...
    B2 = beta_2 + d_beta_2_male * is_m + d_beta_2_temp * temp 
    # expected otolith size next year
