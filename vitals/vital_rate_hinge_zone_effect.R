...
g_z1z_1 <- function(z1, z0, u, a, is_m, m_par) {
  with(m_par, {
    # intercept
    IN = alpha + d_alpha_male * is_m + d_alpha_zone * zone  + u + u_year
    # threshold
    TR = eta + d_eta_male * is_m + d_eta_zone * zone
    # pre-threshold slope
    B1 = beta_1 + d_beta_1_male * is_m + d_beta_1_zone * zone
...
g_z1z_2 <- function(z1, z0, u, a, is_m, m_par) {
  with(m_par, {
    ...
    B2 = beta_2 + d_beta_2_male * is_m + d_beta_2_zone * zone
    # expected otolith size next year
